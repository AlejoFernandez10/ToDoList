...
const TasksCard = ({id, title, description, priority, activeDay}) => {
  ...
  const deleteTask = (id) => {
    ...
    toast.success('Tasks deleted succesfully', {
      position: "top-right",
      autoClose: 2500,      
      closeOnClick: true,
      pauseOnHover: true,
  ...
  return (
    <section className='bg-gray-700 task-card  w-[93%] max-w-[400px]  flex flex-col py-4 px-5 rounded-xl gap-2'>
      ...
      <div className='w-full flex justify-between gap-2'>
        ...
        <h3 className='text-lg font-semibold'> {titleValue !== title ? titleValue : title}</h3>
        }
      ...
      <div className='flex flex-col w-full pt-5'>
            ...
            <button className='self-end' onClick={()=>setActiveEdit(false) }>Save</button>
      </div>
            : 
            <div className='flex w-full pt-5 justify-between items-center '>
              <p className='text-sm text-gray-300'>{description !== descriptionValue ? descriptionValue : description} </p>
              <BsTrash onClick={()=> deleteTask(id)} className='text-red-500 cursor-pointer transition-all duration-200 hover:text-red-700' />
            </div>
      } 
    </section>
  )
